...
| `SHIELD_KNOWN_BAD_URL` | — | URL to fetch `known_bad.json` from at startup and on each refresh interval. When set, the server keeps the list current without redeployment. Cipher-OSS publishes the list at `https://raw.githubusercontent.com/Cipher-OSS/cipher-shield/master/data/known_bad.json` — this is updated between releases when new threats are confirmed. |
